...
- 認証 (2 App 構成・詳細は `meta_accounts.yaml`):
  - **App A** → `tokens/meta/{id}.json` = FB+IG (FB ログイン経路・Page token 1本)。token JSON に
    **無期限ユーザートークン**を master 鍵として保管し、auto_post.yml が**毎 run その場で Page トークンを
    再生成**する (`refresh_meta_token.py --force`)。**PAT も app secret も再OAuth も不要・ユーザー作業ゼロ**。
    Secret = `META_TOKEN_<ID>` (Page トークンは再生成するので Secret 書き戻し無し)
...
| `meta_token_refresh` 失敗 / `IG 連携が確認できない` | 無期限ユーザートークン失効 (パスワード/2FA変更等) or IG↔Page 連携断 | Graph API Explorer でユーザートークン再取得 → `python3 -c` で fb_exchange_token して無期限化 → `gh secret set META_TOKEN_<ID>`。IG連携断なら IG アプリで Page 再連携 |
...
| `external_skills/meta-uploader/scripts/refresh_threads_token.py` | Threads 60日トークン自動更新 (auto_post.yml が毎 run 実行・要 PAT) |
| `external_skills/meta-uploader/scripts/refresh_meta_token.py` | FB+IG Page トークンを無期限ユーザートークンから毎 run 再生成 (PAT 不要) |
